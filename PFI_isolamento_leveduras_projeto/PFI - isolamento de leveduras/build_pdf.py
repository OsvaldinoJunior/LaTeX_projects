...
def check_dependencies():
    ...
    if not shutil.which("pdflatex"):
        error("pdflatex não encontrado.")
        print("  → sudo apt-get install texlive texlive-lang-portuguese texlive-fonts-extra texlive-latex-extra")
        sys.exit(1)
    log("Dependências OK.")


def find_projects():
    """Encontra todos os main.tex excluindo .git e output/."""
    projects = []
    for root, dirs, files in os.walk(SCRIPT_DIR):
        dirs[:] = [d for d in dirs if d not in (".git", "output")]
